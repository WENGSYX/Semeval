...
CFG = { #训练的参数配置
    'fold_num': 5, #五折交叉验证
    'seed': 42,
    'model': 'deberta_v3_large_fintune', #预训练模型
...
def get_data(df):
    ...
    return data
en = pd.read_csv(r'test_subtask-2\En-Subtask2-test.tsv',sep='\t')
fr = pd.read_csv(r'test_subtask-2\Fr-Subtask2-test.tsv',sep='\t')
it = pd.read_csv(r'test_subtask-2\It-Subtask2-test.tsv',sep='\t')
...
valid_en_set = MyDataset(en)
valid_fr_set = MyDataset(fr)
valid_it_set = MyDataset(it)

valid_en_loader = DataLoader(valid_en_set, batch_size=CFG['valid_bs'], collate_fn=collate_fn, shuffle=False,
...
model.load_state_dict(torch.load(r'C:\semeval\3\log\deberta_large_it_fanyi_language_mse_fgm_task2\6_0.8069704810657696_0.8613406737094444_0.7449100048649733_model.pt',map_location='cpu'))
...
valid_en_loader,valid_fr_loader,valid_it_loader = accelerator.prepare(valid_en_loader,valid_fr_loader,valid_it_loader)

log_name = 'test_deberta_large_task2'
path = log_start(log_name)
datas = []
epoch=0
val_loss, val_acc, pred = test_model(model, valid_en_loader,epoch)
pred = ((torch.stack(pred).cpu().numpy()*6)+1).tolist()
for i in range(len(pred)):
    datas.append({'ID':en[i]['id'],'Score':pred[i]})
val_loss, val_acc, pred = test_model(model, valid_fr_loader, epoch)
pred = ((torch.stack(pred).cpu().numpy()*6)+1).tolist()
for i in range(len(pred)):
    datas.append({'ID':fr[i]['id'],'Score':pred[i]})
val_loss, val_acc, pred = test_model(model, valid_it_loader, epoch)
pred = ((torch.stack(pred).cpu().numpy()*6)+1).tolist()
for i in range(len(pred)):
    datas.append({'ID':it[i]['id'],'Score':pred[i]})
