...
CFG = { #训练的参数配置
    ...
    'epochs': 16,
    'train_bs': 24, #batch_size，可根据自己的显存调整
    'valid_bs': 24,
    'lr': 1e-5, #学习率
    'num_workers': 0,
...
def get_data(df):
    ...
    for i in range(len(df)):
        data.append({'id':df.iloc[i]['ID'],'sen':df.iloc[i]['Sentence'],'label':1})
    return data
en = pd.read_csv(r'C:\semeval\3\SemEval2022Task3-main\data\test\official_test_sets\subtask-1\En-Subtask1-test.tsv',sep='\t')
fr = pd.read_csv(r'C:\semeval\3\SemEval2022Task3-main\data\test\official_test_sets\subtask-1\Fr-Subtask1-test.tsv',sep='\t')
it = pd.read_csv(r'C:\semeval\3\SemEval2022Task3-main\data\test\official_test_sets\subtask-1\It-Subtask1-test.tsv',sep='\t')
...
valid_en_loader = DataLoader(en_set, batch_size=CFG['valid_bs'], collate_fn=collate_fn, shuffle=False,
                          num_workers=CFG['num_workers'])
valid_fr_loader = DataLoader(fr_set , batch_size=CFG['valid_bs'], collate_fn=collate_fn, shuffle=False,
                          num_workers=CFG['num_workers'])
valid_it_loader = DataLoader(it_set , batch_size=CFG['valid_bs'], collate_fn=collate_fn, shuffle=False,
                          num_workers=CFG['num_workers'])
best_acc = 0

from transformers import DebertaV2ForSequenceClassification
model = DebertaV2ForSequenceClassification.from_pretrained(CFG['model'],num_labels = 2)  # 模型
model.load_state_dict(torch.load(r'C:\semeval\3\log\deberta_large_enandfr_all_fgm_task1\2_1.0_1.0_0.9492989895463189_model.pt',map_location='cpu'))
...
val_en_loader,valid_fr_loader,valid_it_loader = accelerator.prepare(valid_en_loader,valid_fr_loader,valid_it_loader)

log_name = 'test_deberta_task1'
path = log_start(log_name)
epoch = 0
datas = []
val_loss, val_acc, pred = test_model(model, val_en_loader,epoch)
pred = torch.stack(pred).argmax(1).cpu().tolist()
for i in range(len(pred)):
    datas.append({'ID':en[i]['id'],'Labels':pred[i]})
val_loss, val_acc, pred = test_model(model, valid_fr_loader, epoch)
pred = torch.stack(pred).argmax(1).cpu().tolist()
for i in range(len(pred)):
    datas.append({'ID':fr[i]['id'],'Labels':pred[i]})
val_loss, val_acc, pred = test_model(model, valid_it_loader, epoch)
pred = torch.stack(pred).argmax(1).cpu().tolist()
for i in range(len(pred)):
    datas.append({'ID':it[i]['id'],'Labels':pred[i]})

datas = pd.DataFrame(datas)
datas.to_csv(path+'/answer.tsv',sep='\t')
