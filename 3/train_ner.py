...
train = pd.read_pickle('ner_train.pk')

valid = pd.read_pickle('ner_valid.pk')
...
def collate_fn(data):
    ...
    for x in data:
        ...
        for token in sen.split(' '):
            text = tokenizer(token)['input_ids'][1:-1]
            if token in x['label']:
                label+=[1]*len(text)
            else:
                label+=[0]*len(text)
        ...
        label += [0]
        len_input = len(input_id)
        if len_input < CFG['max_len']:
            input_id+=[tokenizer.pad_token_id] * (CFG['max_len']-len_input)
            label += [-100] * (CFG['max_len']-len_input)
        else:
...
def train_model(model, fgm,pgd,train_loader,epoch):  # 训练一个epoch
    ...
    for step, (input_ids,y) in enumerate(tk):

        output = model(input_ids, labels=y)
        loss = output.loss
        ...
        for i in range(len(y)):
            for p in range(len(y[i])):
                if y[i][p] != -100:
                    if y[i][p] == output.logits[i][p].argmax():
                        accs.update(1)
                    else:
                        accs.update(0)
...
def test_model(model, val_loader,epoch):  # 验证
    ...
    with torch.no_grad():
        ...
        for idx, (input_ids, y) in enumerate(tk):
...
model = DebertaV2ForTokenClassification.from_pretrained(CFG['model'],num_labels = 2)  # 模型
...
for epoch in range(CFG['epochs']):
    ...
    model_name = path+'/{}_{}_model.pt'.format(epoch,val_acc,val_loss)
