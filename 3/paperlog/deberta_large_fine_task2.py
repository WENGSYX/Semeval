...
CFG = { #训练的参数配置
    'fold_num': 5, #五折交叉验证
    'seed': 42,
    'model': r'C:\semeval\3\paperlog\deberta_large_task1\3_0.9600671851829434_0.946022584393098_0.9264842039525519_model', #预训练模型
...
class Debertafortask2(DebertaV2PreTrainedModel):
    def __init__(self, config: DebertaV2Config):
        ...
        self.clas = nn.Linear(config.hidden_size, 1)
        self.init_weights()
    ...
    def forward(
        ...
        logits = self.clas(sequence_output.mean(1))[:,0]
        return logits
...
log_name = 'deberta_large_fine_task2'
...
CFG = { #训练的参数配置
    'fold_num': 5, #五折交叉验证
    'seed': 42,
    'model': r'C:\semeval\3\paperlog\deberta_large_enandfr_task1\0_0.9390666299073758_0.9203890209955696_0.9214658266441287_model', #预训练模型
    'max_len': 40, #文本截断的最大长度
    'epochs': 8,
    'train_bs': 16, #batch_size，可根据自己的显存调整
    'valid_bs': 16,
...
CFG = { #训练的参数配置
    'fold_num': 5, #五折交叉验证
    'seed': 42,
    'model': r'deberta_v3_large_fintune_by2', #预训练模型
    'max_len': 40, #文本截断的最大长度
    'epochs': 8,
    'train_bs': 16, #batch_size，可根据自己的显存调整
    'valid_bs': 16,
