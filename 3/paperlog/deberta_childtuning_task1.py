...
CFG = { #训练的参数配置
    ...
    'max_len': 40, #文本截断的最大长度
    'epochs': 16,
    'train_bs': 40, #batch_size，可根据自己的显存调整
    'valid_bs': 40,
    'lr': 1e-5, #学习率
    'num_workers': 0,
...
def get_test(data):
    ...
    for p in range(len(data)):
        item = data.iloc[p]
        d.append({'sen':item['Sentence'],'label':item['Labels']})
    return d

train = pd.read_pickle('en_subtask1_train.pk')
train.extend(pd.read_pickle('en_subtask1_valid.pk'))


train.extend(pd.read_pickle('fr_subtask1_train.pk'))
train.extend(pd.read_pickle('fr_subtask1_valid.pk'))

train.extend(pd.read_pickle('it_subtask1_train.pk'))
train.extend(pd.read_pickle('it_subtask1_valid.pk'))

valid = get_test(pd.read_csv('En-Subtask1-labels.tsv',sep='\t'))
valid_fr = get_test(pd.read_csv('Fr-Subtask1-labels.tsv',sep='\t'))
valid_it = get_test(pd.read_csv('It-Subtask1-labels.tsv',sep='\t'))

r = pd.read_csv('def_fr_reverso_result.csv')
dingyi=[]
for i in range(len(r)):
    dingyi.append([r.iloc[i]['name'],' '+r.iloc[i]['name']+' ',' '+r.iloc[i]['name']+'  [UNK] '+r.iloc[i]['def']+' [UNK] '])
...
log_name = 'deberta_childtuning_task1'
...
for epoch in range(CFG['epochs']):
    ...
    log_fr,lfr = compute_metrics_task1(torch.stack(pred).argmax(1).cpu().numpy(),true_fr)
    log(['Frensh']+log_fr,path)
    val_loss, val_acc, pred = test_model(model, valid_it_loader, epoch)
    log_it,lit = compute_metrics_task1(torch.stack(pred).argmax(1).cpu().numpy(),true_it)
    log(['It']+log_it,path)
    model_name = path+'/{}_{}_{}_{}_model.pt'.format(epoch,l1['F1macro'],lfr['F1macro'],lit['F1macro'])
...
"""
"""
train = pd.read_pickle('fr_subtask1_train.pk')
train.extend(pd.read_pickle('fr_subtask1_valid.pk'))
"""
train = pd.read_pickle('it_subtask1_train.pk')
train.extend(pd.read_pickle('it_subtask1_valid.pk'))
...
log_name = 'deberta_only_enandfr_childtuning_task1'
path = log_start(log_name)
