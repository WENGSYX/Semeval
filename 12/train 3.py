...
CFG = { #训练的参数配置
    'fold_num': 5, #五折交叉验证
    'seed': 42,
    'model': r'C:\semeval\12\deberta_v3_large', #预训练模型
        ...
        for idx, (input_ids, attention_mask,labels) in enumerate(tk):
            ...
            for logitsn in range(len(logits)):
                ...
                for ln in range(len(t)):
                    ...
                    last_l = l
                primary,symbol,ordered=[],[],[]
                primarys_true,symbols_true,ordereds_true=[],[],[]
                for ln in range(len(la)):
                    l = la[ln]
                    if l != last_l:
                        if primary != []:
                            primarys_true.append(tokenizer.decode(primary))
                            primary = []
                        if symbol != []:
                            symbols_true.append(tokenizer.decode(symbol))
                            symbol = []
                        if ordered != []:
                            ordereds_true.append(tokenizer.decode(ordered))
                            ordered = []
                    if l == 1:
                        primary.append(input_ids[logitsn][ln])
                    if l == 2:
                    ...
                    if l == 3:
                        ordered.append(input_ids[logitsn][ln])
                    last_l = l
                primary_f1 = get_F1(primarys_true,primarys)
                symbol_f1 = get_F1(symbols_true,symbols)
                ordered_f1 = get_F1(ordereds_true,ordereds)
                f1s.update((primary_f1+symbol_f1+ordered_f1)/3)
                primary_f1s.update(primary_f1)
                symbol_f1s.update(symbol_f1)
                ordered_f1s.update(ordered_f1)
            if idx % 10 == 0:
