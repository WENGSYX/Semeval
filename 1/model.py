...
from transformers import BertModel, BertConfig,DebertaV2PreTrainedModel,DebertaV2Config, DebertaV2Model
...
class SimDeberta(DebertaV2PreTrainedModel):
    def __init__(self, config: DebertaV2Config):
        super().__init__(config)
        self.deberta = DebertaV2Model(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.pooler = nn.AvgPool1d(3)
        self.init_weights()
    ...
    def forward(
        ...
        sequence_output = outputs.last_hidden_state.mean(1)
...
class CPTModel(CPTPretrainedModel):
    def __init__(self, config: CPTConfig):
        ...
        encoder_config = DebertaV2Config(
            ...
            max_position_embeddings=config.max_encoding_position_embeddings,
        max_relative_positions=256
        )
        config.vocab_size = encoder_config.vocab_size
        self.deberta = DebertaV2Model(encoder_config)
        self.dropout = nn.Dropout(0.1)
        self.pooler = nn.AvgPool1d(3)
        self.shared = self.deberta.get_input_embeddings()
    ...
    def set_input_embeddings(self, value):
        self.shared = value
        self.deberta.set_input_embeddings(self.shared)
    ...
    def forward(
        ...
        #     )
        #     encoder_outputs = encoder_outputs.last_hidden_state.mean(1)
        ...
        encoder_hidden_states = torch.cat([encoder_hidden_states]*3,axis=1)
        # decoder outputs consists of (dec_features, past_key_value, dec_hidden, dec_attn)
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask,
            encoder_hidden_states=torch.stack([encoder_hidden_states]*1024,axis=1), ## 4*256
