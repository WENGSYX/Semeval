...
train = pd.read_pickle('train_score.pk')
valid = pd.read_pickle('dev_score.pk')
...
def collate_fn(data):
    ...
    for x in data:
        for w in range(len(x['word'])):
            text = tokenizer(str(x['word'][w]),x['sen'].replace('[MASK]',str(x['word'][w])), padding='max_length', truncation=True, max_length=CFG['max_len'])
            input_ids.append(text['input_ids'])
            attention_mask.append(text['attention_mask'])
            token_type_ids.append(text['token_type_ids'])
            labels.append((x['label'][w]-1)/4)
...
def train_model(model, fgm,pgd,train_loader,epoch):  # 训练一个epoch
    ...
    for step, (input_ids, attention_mask, token_type_ids,y) in enumerate(tk):
        ...
        acc = (abs(output - y)).sum().item() * 4 / y.size(0)
        accs.update(acc)
    ...
    with open(path+'/valid_label_{}.tsv'.format(epoch),'w',encoding='utf-8') as f:
        ...
        for i in range(len(pred)):
            ...
            if num<=2.75:
                ...
                value = 1
            f.write(['PLAUSIBLE', 'NEUTRAL', 'IMPLAUSIBLE'][value])
            f.write('\n')
            n+=1
            if n==6:
                n = 1
...
model = DebertaForMultipleChoice.from_pretrained(CFG['model'])  # 模型
