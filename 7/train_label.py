...
CFG = { #训练的参数配置
    'fold_num': 5, #五折交叉验证
    'seed': 2002,
    'model': 'deberta_v3', #预训练模型
    'max_len': 512, #文本截断的最大长度
    'epochs': 16,
    'train_bs': 15, #batch_size，可根据自己的显存调整
    'valid_bs': 15,
    'lr': 1e-5, #学习率
    'num_workers': 0,
...
train = pd.read_pickle('train_label.pk')
valid = pd.read_pickle('dev_label.pk')
...
def collate_fn(data):
    ...
    for x in data:
        text = tokenizer(str(x['word']),text_pair=x['sen'], padding='max_length', truncation=True, max_length=CFG['max_len'])
...
def test_model(model, val_loader,epoch):  # 验证
    ...
    with torch.no_grad():
        ...
        for idx, (input_ids, attention_mask, token_type_ids, y) in enumerate(tk):
            ...
            output = torch.sigmoid(model(input_ids, attention_mask, token_type_ids).logits)
    ...
    with open(path+'/valid_label_{}.tsv'.format(epoch),'w',encoding='utf-8') as f:
        n = 1
        p = 0
        for i in range(len(pred)):
            f.write('{}_{}'.format(p,n))
            f.write('\t')
            num = pred[i].item()
            f.write(['PLAUSIBLE', 'NEUTRAL', 'IMPLAUSIBLE'][num])
            f.write('\n')
            n+=1
            if n==6:
                n = 1
                p +=1
    with open(path+'/valid_score_{}.tsv'.format(epoch),'w',encoding='utf-8') as f:
        n = 1
        p = 0
        for i in range(len(pred)):
            f.write('{}_{}'.format(p,n))
            ...
            if n==6:
                n = 1
                p +=1
    s1 = score('ClarificationTask_DevLabels_Oct22a.tsv',path+'/valid_label_{}.tsv'.format(epoch),'classification')
    s2 = score('ClarificationTask_DevScores_Oct22a.tsv', path+'/valid_score_{}.tsv'.format(epoch), 'ranking')
    log(['测试损失：{}'.format(str(losses.avg)),'测试准确率：{}'.format(accs.avg),'结束本轮测试','Score测试分数:{}'.format(s1),'分类测试分数:{}'.format(s2)],path)
    return losses.avg, accs.avg,s1,s2
...
model = DebertaV2ForSequenceClassification.from_pretrained(CFG['model'],num_labels = 3)  # 模型
...
log_name = 'baseline_label_onebyone'
path = log_start(log_name)

for epoch in range(CFG['epochs']):

    train_loss = train_model(model, fgm, pgd, train_loader,epoch)
    val_loss, val_acc, s1,s2 = test_model(model, val_loader,epoch)
    model_name = path+'/{}_{}_{}_model.pt'.format(epoch,s1,s2)
